...
export async function createOrg(_prev: CreateOrgState | null, formData: FormData): Promise<CreateOrgState> {
  const { userId } = await auth();
  if (!userId) return { error: "You must be signed in to create an organisation." };

  const name = String(formData.get("name") ?? "").trim();
  if (name.length < 2) return { error: "Organisation name must be at least 2 characters." };
