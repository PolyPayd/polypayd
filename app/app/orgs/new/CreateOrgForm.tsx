...
type Props = { createOrg: CreateOrgAction };

export function CreateOrgForm({ createOrg }: Props) {
  const [state, formAction, isPending] = useActionState<CreateOrgState | null, FormData>(createOrg, null);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      >
        {isPending ? "Creating…" : "Create and go to wallet"}
      </button>
    </form>
  );
}
