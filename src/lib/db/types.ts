...
// ---------------------------------------------------------------------------
// INSERT TYPES
// Fields with DB defaults (id, created_at, status, etc.) or that are nullable
// are optional. NOT NULL fields without a default are required.
// ---------------------------------------------------------------------------
...
// Supabase's GenericSchema requires Tables, Views, and Functions.
// Each GenericTable requires Row, Insert, Update, and Relationships.
// Missing any of these causes Schema to resolve to `never` inside the client,
// turning every .insert()/.upsert()/.update() parameter into `never`.
