...
import { generateClaimCode } from "@/src/lib/claim-codes";
...
const bodySchema = z.object({
  ...
  total_pence:  z.number().int().positive().max(1_000_000), // max £10,000
...
export async function POST(req: Request): Promise<Response> {
  ...
  const { name, description, mode, closing_mode, total_pence, max_recipients, recipients } = parsed.data;

  // Validate custom amounts consistency
  if (mode === "custom_amounts") {
    if (!recipients?.length) {
      return Response.json(
        { error: "validation_error", message: "Custom amounts mode requires at least one recipient" },
        { status: 422 }
      );
    }
    const recipientsTotal = recipients.reduce((sum, r) => sum + r.amount_pence, 0);
    if (recipientsTotal !== total_pence) {
      return Response.json(
        { error: "validation_error", message: "Recipients total must equal total_pence" },
        { status: 422 }
      );
    }
  }
  ...
  // Insert batch with claim_code uniqueness retry
  ...
  if (!batchId) {
    return Response.json({ error: "internal", message: "Failed to generate unique claim code" }, { status: 500 });
  }

  // Insert claim_links row so the batch has a public claim URL
  await db.from("claim_links").insert({ batch_id: batchId });

  // Insert batch_slots for custom amounts
  if (mode === "custom_amounts" && recipients?.length) {
    ...
    if (slotError) {
      ...
      // Non-fatal for now — batch was created; log and continue
    }
  }

  // Create batch_escrow ledger account
  ...
  // Record fee reservation against fees ledger account
  if (escrowAccount) {
    ...
    await db.from("ledger_entries").insert([
      ...
      {
        account_id:     (escrowAccount as { id: string }).id,
        direction:      "debit",
        amount_pence:   feePence,
        transaction_id: txId,
        reference:      `fee_reservation:${batchId}`,
  ...
  void logAuditEvent({
    ...
    entityType: "batch",
    entityId:   batchId,
    metadata:   { name, mode, total_pence, fee_pence: feePence, max_recipients },
  });
