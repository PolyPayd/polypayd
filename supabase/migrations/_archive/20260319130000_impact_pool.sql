-- Impact pool: 1% of platform fee revenue allocated to __impact__ wallet.
-- Platform fee is computed in batch RPCs (basis points); impact is 1% of that fee only.

-- Ledger of impact allocations (one row per source ledger transaction).
...
comment on table public.impact_ledger is
  'Records impact pool credits: 1% of platform fee per batch ledger transaction.';

-- Future: payouts from impact pool to charities / programs.
...
comment on table public.impact_distributions is
  'Planned: outbound impact payouts (charities). Not wired in batch RPCs yet.';

-- Idempotent: credit __impact__ from __system__ using 1% of platform fee; insert impact_ledger once per source txn.
...
begin
  ...
  v_impact := round(p_platform_fee * 0.01, 2);
  if v_impact <= 0 then
    return;
  end if;
  ...
  from public.wallets
  where user_id = '__system__' and currency = v_cur
  for update;
  ...
  if v_system_wallet_id is null or v_impact_wallet_id is null then
    raise exception 'Impact allocation failed: system or impact wallet missing';
  end if;
  ...
  if not found then
    raise exception 'Impact allocation failed: insufficient balance in system clearing wallet';
  end if;
...
comment on function public.apply_impact_from_platform_fee(uuid, numeric, text) is
  'Moves one percent of p_platform_fee from __system__ to __impact__; records impact_ledger (idempotent per source_transaction_id).';
