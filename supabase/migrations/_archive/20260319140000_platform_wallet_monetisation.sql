-- Platform monetisation: __platform__ wallet, ledger fee columns, fee + impact flow.
-- Platform fee default: 150 bps (1.5%). Impact: 1% of platform_fee moves __platform__ → __impact__ (apply_impact_from_platform_fee).

-- 1) Ledger transaction metadata (fee charged on that payout)
...
comment on column public.ledger_transactions.platform_fee is
  'Platform fee (currency units) charged on this ledger transaction.';
comment on column public.ledger_transactions.fee_bps is
  'Basis points used when computing platform_fee for this row.';

-- 2) Impact slice: debit __platform__ (not __system__) after fee is credited there
...
-- 3) Bulk Send (standard batch run): debit sender (principal+fee), credit __platform__ (fee), credit __system__ (principal)
...
begin
  ...
  update wallets
  set current_balance = current_balance - v_total_debit,
      updated_at = now()
  where id = v_sender_wallet_id
    and current_balance >= v_total_debit
  ...
  loop
    ...
    if v_should_fail then
      ...
      if v_txn_id is not null then
        update wallets
        set current_balance = current_balance - v_line_total,
            updated_at = now()
        where id = v_sender_wallet_id;

        if v_line_fee > 0 then
          update wallets
          set current_balance = current_balance + v_line_fee,
              updated_at = now()
          where id = v_platform_wallet_id;
