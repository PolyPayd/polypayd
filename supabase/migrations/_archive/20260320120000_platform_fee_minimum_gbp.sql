-- Enforce minimum platform fee £1.00 on payout RPCs (live deploy).
-- Replaces: public.process_standard_batch_run, public.process_claimable_batch_payout, public.process_standard_batch_retry_failed
-- Rule: calculated_fee = round(principal * fee_bps / 10000, 2); platform_fee = greatest(calculated_fee, 1.00); fee_bps = 150.
-- Impact: 1% of final platform_fee (apply_impact_from_platform_fee).
-- Source of truth: supabase/apply-process-standard-batch-payout.sql (same bodies).
