-- Wallets: one per user per currency (user_id = Clerk ID, text for app compatibility)
...
-- Ledger: one row per logical transaction (e.g. one batch payout)
...
comment on column ledger_transactions.idempotency_key is 'Prevents duplicate posting (e.g. batch-payout-<batch_id>).';

-- Ledger entries: immutable double-entry rows
...
-- Payouts: one per recipient per batch payout
...
comment on table payouts is 'One row per payout to a recipient (batch payout execution).';

-- Batches: who funded this batch (whose wallet to debit)
alter table batches add column if not exists funded_by_user_id text null;
