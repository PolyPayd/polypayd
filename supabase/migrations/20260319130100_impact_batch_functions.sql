-- Depends on: 20260319130000_impact_pool.sql (impact_ledger + apply_impact_from_platform_fee).
-- Replaces batch RPCs to charge optional platform fee (basis points) and allocate 1% of fee to impact pool.
...
begin
  ...
  v_platform_fee := round(v_pending_amount * c_platform_fee_bps / 10000.0, 2);
  ...
  if v_ledger_transaction_id is null then
    ...
    values
      ...
      (v_ledger_transaction_id, v_system_wallet_id, v_total_debit, 'credit', 'batch_run', p_batch_id);
...
begin
  ...
  if v_platform_fee > 0 then
    insert into wallets (user_id, currency, current_balance, updated_at)
    values ('__system__', v_currency, 0, now())
    on conflict (user_id, currency) do nothing;
    ...
    insert into ledger_entries (transaction_id, wallet_id, amount, entry_type)
    values (v_txn_id, v_system_wallet_id, v_platform_fee, 'credit');
