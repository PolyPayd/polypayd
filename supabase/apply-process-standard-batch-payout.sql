-- Standalone SQL for wallet + ledger posting for CSV/manual (standard) batches.
-- If you cannot run migrations, paste this into Supabase SQL Editor.
--
-- Prerequisites: apply-impact-pool.sql (impact_ledger + ledger fee columns + apply_impact), then this file.
-- Platform fee: c_platform_fee_bps = 150 (1.5%); minimum £1.00 via greatest(calculated_fee, 1.00).
-- Deploy to Supabase: run this file, or migration 20260320120000_platform_fee_minimum_gbp.sql (replaces all three RPCs).

-- RPC: process_standard_batch_run(p_batch_id uuid)
-- RPC: process_standard_batch_retry_failed(p_batch_id uuid)
...
-- Migrated from 20260319140000_platform_wallet_monetisation.sql (process_standard_batch_run + retry_failed)
